...
class Interpreter implements Expr.Visitor<Object>,
  ...
  void pushInner(LoxCallable innerCallable) {
    innerStack.push(innerCallable);
  }
  ...
  public Object visitSuperExpr(Expr.Super expr) {
    ...
    List<LoxClass> superclasses =
        (List<LoxClass>) environment.getAt(distance, "super");

    LoxInstance object =
        (LoxInstance) environment.getAt(distance - 1, "this");

    LoxFunction method = findMethod(superclasses, expr.method.lexeme);
    if (method == null) {
  ...
  public Void visitClassStmt(Stmt.Class stmt) {
    ...
    LoxClass klass = new LoxClass(stmt.name.lexeme,
        superclasses, methods, classMethods);
  ...
  private LoxFunction findMethod(List<LoxClass> superclasses, String name) {
